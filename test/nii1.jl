...
function nii1_tests(img::AbstractArray)
    @testset "NIfTI Interface" begin
        ...
        @test phasedim(img) == 3
        @test slicedim(img) == 4
        @test qformcode(img)  == CoordinateSpace{:scanner}()
    ...
    @testset "ImageFormat Interface" begin
        @test all(spatialunits(img) .== u"mm")
    ...
    @testset "Images Interface" begin
        ...
        @test spatialorder(img) == (:right_to_left, :posterior_to_anterior, :inferior_to_superior)
    end
end
...
@testset ".nii+mmap" begin
    img = NIfTI.load(File(format"NII", NII), mmap=true)
    nii1_tests(img)
end

@testset ".nii.gz" begin
    img = NIfTI.load(File(format"NII", GZIPPED_NII))
    nii1_tests(img)
end
...
@testset "save" begin
    ...
    img = NIfTI.load(File(format"NII", TEMPNII_FILE))
    nii1_tests(img)
end

@testset ".hdr" begin
    img = NIfTI.load(File(format"NII", HDR), mmap=true)
    hdr1_tests(img)
end

@testset ".hdr.gz" begin
    img = NIfTI.load(File(format"NII", GZIPPED_HDR))
    hdr1_tests(img)
end
...
@testset "orientation" begin
    ...
    # LR
    img = NIfTI.load(File(format"NII", ALR))
    @test size(img) == (91,109,91)
    @test spatialorder(img) == (:right_to_left, :posterior_to_anterior, :inferior_to_superior)

    # RL
    img = NIfTI.load(File(format"NII", ARL))
    @test size(img) == (91,109,91)
    @test spatialorder(img) == (:left_to_right, :posterior_to_anterior, :inferior_to_superior)
end
